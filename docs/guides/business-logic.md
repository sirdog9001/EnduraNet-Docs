In the context of EnduraNet and the `can_bypass_validate` permission, *business logic* is any validation rule that exists for a purpose that *is not* to protect the database from errors.
